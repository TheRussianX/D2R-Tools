...
#can't use relative paths when batch setting shaders, so I put another relative paths setting after that
fbxfiles = glob.glob ('d:\Download\D2R_test\items\_to_export\\**\\*.fbx', recursive=True)
for f_file in fbxfiles:
    fbxpath = os.path.split(f_file)[0]
    model_name = os.path.split(f_file)[1].split('.')[0]
    blend_path = fbxpath+'\\New_testfile.blend'
    abs_tex_path = fbxpath+'\\textures'
    print(f_file)
    print(blend_path)
    print(abs_tex_path)
    bpy.ops.wm.open_mainfile(filepath=blend_path)
    bpy.ops.import_scene.fbx(filepath=f_file)

    #setting relative paths for textures
    ...
    #fixing shaders; warning - some models have ORM files for fur (feathers)
    ...
    bpy.ops.wm.save_as_mainfile(filepath=fbxpath+'\\'+model_name+'.blend')
