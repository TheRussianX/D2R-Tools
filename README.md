...
Warning! This method sometimes merge 2 differents LODs (level of detail) of the same mesh, both present in the same LOD0.model file, if the internal name of the LODs are the same (without numbers). If all your mesh names start with a number (example 0001_) you're good. If there are some meshes without the number prefix those might be 2 fused LODs. I noticed this problem with CorruptedRogue outfit, Steleton Outfit, Pantherwoman outfit.  
